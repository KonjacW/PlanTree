...
export type PromptFileCopier = (input: { snapshot: PlanSnapshot; chain: ExecutionChain }) => Promise<{ fileName?: string }>;
...
export function PlanTreeWindow({ plan, toolCaller, promptFileCopier, webMode = false }: PlanTreeWindowProps & { toolCaller?: ToolCaller; promptFileCopier?: PromptFileCopier; webMode?: boolean }) {
  ...
  const copyPromptFile = async () => { if (loading) return; if (!toolCaller || !promptFileCopier) { setError("提示文件复制功能尚未连接。"); return; } setLoading(true); setError(null); try { const response = executionFromToolResult(await toolCaller("compile_execution_chain", {})); if (!response) throw new Error("工具响应未包含执行链。"); applySnapshot(response.snapshot); setExecutionChain(response.chain); const remaining = response.chain.tasks.filter((task) => task.status !== "completed"); if (!remaining.length) { setChainOpen(true); setFeedback("执行链中的任务已全部完成。"); return; } const copied = await promptFileCopier({ snapshot: response.snapshot, chain: response.chain }); setChainOpen(false); setFeedback(`已将 ${copied.fileName ?? "plantree-prompt.md"} 文件复制到剪贴板；请粘贴到目标 Codex 对话并发送。`); } catch (reason) { setError(reason instanceof Error ? reason.message : "无法复制执行提示文件。"); } finally { setLoading(false); } };
  ...
  const performPromptAction = (action: PendingPromptAction) => {
    ...
    else if (action.type === "copy") { closePrompt(); setHelpOpen(false); setChainOpen(false); setPruneIds(null); void copyPromptFile(); }
