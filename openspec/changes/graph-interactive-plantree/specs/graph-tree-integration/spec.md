...
### Requirement: 侧栏图形与 Codex 执行交接
系统 MUST 只在本地侧栏 Web 显示图形任务树，不注册内嵌任务树资源。节点内容、撤销与重做结果 MUST 通过服务端快照保持一致。`render_plan_tree` MUST 记录创建任务树的 Codex `threadId`。侧栏点击自动执行 MUST 写入包含计划 ID 和快照版本的递增请求，并 MUST 恢复该 `threadId` 后再领取任务。

#### Scenario: 侧栏一次点击启动执行
- **WHEN** 用户在已确认的侧栏任务树点击“开始自动执行”
- **THEN** 创建该树的 Codex 对话出现新回合并启动完整剩余执行链，用户不需要保持旧回合等待、复制或逐项粘贴提示词
